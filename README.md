...
| Fonts | Inter (vendored) | Clean, readable |

No build tools, bundlers, or frameworks are required. `vendor/` contains the pinned Leaflet client, Supabase JS client, Font Awesome icon fonts, and the Inter web font, so the app has no runtime CDN dependency at all — it loads and works fully offline once cached by the service worker.
...
├── vendor/               # Pinned Leaflet + Supabase JS client + Font Awesome + Inter font
├── assets/
